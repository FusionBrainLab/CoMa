...
class FileHandler:
    ...
    def upload_files_to_s3(self, files: List[str]) -> tuple[List[str], str, str]:
        import base64
        ids_list: List[str] = []
        files_bytes = [base64.b64decode(file) for file in files]
        for file in files_bytes:
            ids_list.append(self._put_file_post_request(file=file))

        return self._validate_files_from_request(
            id_list=files, results=ids_list, upload=True
        )