...
class MassingMeshIoU(SampleMetric):
    def __init__(self, *, gt_col: str,
                        gt_type: Literal["polygon", "mesh"],
                        pred_col: str,
    ...
    def __call__(self, *, sample: Dict[str, Any]) -> float:
        def get_mesh(massing, massing_type):
            if massing_type == "polygon":
                return self.mesh_creator(obj=massing)
            elif massing_type == "mesh":
                return trimesh.load(massing)
        ...
        for gt in total_gt_massings:
            ...
            gt_mesh = get_mesh([gt], self.gt_type)
            pred_mesh = get_mesh([pred], self.pred_type)
            ...
            intersection = gt_mesh.intersection(pred_mesh)
            union = gt_mesh.union(pred_mesh)
