...
class MassingMeshCompiler(MeshCompiler):
    def __init__(self) -> None:
        ...
        def extrude_shapely_polygon(polygon: Polygon, base_elev: float, height: float) -> trimesh.Trimesh:
            ...
            mesh = trimesh.creation.extrude_polygon(poly_2d, height=height)
    ...
    def __call__(self, *, obj: Any) -> trimesh.Trimesh:
        ...
        for m in obj:
            for extrusion in m["massing"]:
                ...
                polygons = [[(p[0], p[1]) for p in polygon] for polygon in polygons]
                shapely_geom = self.polygons_converter(polygons=polygons)
                layer_meshes = []
